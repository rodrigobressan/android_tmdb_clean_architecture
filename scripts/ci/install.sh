...
# Assemble mobile artifacts (Debug and QA build types)
./gradlew :mobile:assembleDebug --stacktrace --no-daemon
./gradlew :mobile:assembleQa --stacktrace --no-daemon

# Assemble tv artifacts (Debug and QA build types)
...
# Assemble tv artifacts for Android Instrumentation Test (later deployed to Firebase Test Lab)
./gradlew :tv:assembleAndroidTest
